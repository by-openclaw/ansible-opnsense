...
"""Ansible module for OPNsense GRE tunnel interface management."""

from __future__ import annotations

DOCUMENTATION = r"""
---
module: opnsense_if_gre
short_description: Manage OPNsense GRE tunnel interfaces
version_added: "0.2.0"
description:
  - Create, update, or delete GRE (Generic Routing Encapsulation) tunnels on OPNsense.
  - Thin wrapper around lib-opnsense IfGreManager.ensure().
...
options:
  ...
  descr:
    ...
    default: ""
  state:
    description: Desired state of the GRE tunnel.
    type: str
...
- name: Create a GRE tunnel
  by_systems.opnsense.opnsense_if_gre:
    host: "{{ opn_host }}"
    key: "{{ opn_key }}"
    secret: "{{ opn_secret }}"
    tunnel_local_addr: "198.51.100.1"
    tunnel_remote_addr: "203.0.113.1"
    descr: "GRE tunnel to remote site"
    state: present
...
def main() -> None:
    ...
    run_module(module, IfGreManager, params)
