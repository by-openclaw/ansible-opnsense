...
"""Ansible module for the OPNsense dnscrypt-proxy service via lib-opnsense."""

from __future__ import annotations

DOCUMENTATION = r"""
---
module: opnsense_dnscrypt_service
short_description: Control the OPNsense dnscrypt-proxy service (os-dnscrypt-proxy)
version_added: "0.5.0"
description:
  - Start, stop or reconfigure the dnscrypt-proxy service on an OPNsense firewall.
  - Thin wrapper around lib-opnsense DnscryptProxyServiceManager.ensure().
  - >
    The service reports C(disabled) while the plugin is switched off in its
    settings (see M(by_systems.opnsense.opnsense_dnscrypt_settings)); a C(running) request
...
EXAMPLES = r"""
- name: Apply changed dnscrypt-proxy settings
...
def main() -> None:
    ...
    run_module(module, DnscryptProxyServiceManager, {})
