...
"""Ansible module opnsense_netflow_service — thin wrapper on lib-opnsense."""

from __future__ import annotations

DOCUMENTATION = r"""
---
module: opnsense_netflow_service
short_description: Control the OPNsense NetFlow / Insight exporter service
version_added: "0.5.0"
description:
  - >
    Start, stop, restart or reconfigure the NetFlow / Insight exporter service — lib-opnsense NetflowServiceManager.ensure().
  - >
    The controller only has status + reconfigure: use C(reconfigured); C(running)/C(stopped) would call endpoints it does not have.
...
EXAMPLES = r"""
- name: Apply the NetFlow / Insight exporter configuration
  by_systems.opnsense.opnsense_netflow_service:
    host: "{{ opn_host }}"
    key: "{{ opn_key }}"
    secret: "{{ opn_secret }}"
    state: reconfigured
...
def main() -> None:
    ...
    if module.params["state"] == "restarted":
        ...
        async def _restart() -> None:
            async with OpnsenseClient(
                ...
                r = await NetflowServiceManager(client).restart(
    ...
    run_module(module, NetflowServiceManager, {})
