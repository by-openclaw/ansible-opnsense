...
"""Ansible module opnsense_mdnsrepeater_service — thin wrapper on lib-opnsense."""

from __future__ import annotations

DOCUMENTATION = r"""
---
module: opnsense_mdnsrepeater_service
short_description: Control the OPNsense mDNS repeater service
version_added: "0.5.0"
description:
  - >
    Start, stop or reconfigure the mDNS repeater service — lib-opnsense MdnsRepeaterServiceManager.ensure().
  - Reports C(disabled) while switched off in its settings.
...
EXAMPLES = r"""
- name: mDNS repeater — apply configuration
  by_systems.opnsense.opnsense_mdnsrepeater_service:
    host: "{{ opn_host }}"
    key: "{{ opn_key }}"
    secret: "{{ opn_secret }}"
    state: reconfigured
...
def main() -> None:
    ...
    from opnsense.managers.services.mdnsrepeater_service import (
        MdnsRepeaterServiceManager,
    )

    run_module(module, MdnsRepeaterServiceManager, {})
