...
"""Ansible module opnsense_kea6_settings — thin wrapper on lib-opnsense."""

from __future__ import annotations

DOCUMENTATION = r"""
---
module: opnsense_kea6_settings
short_description: Manage OPNsense Kea DHCPv6 general settings
version_added: "0.5.0"
description:
  - Manage the C(general) block of Kea DHCPv6 — lib-opnsense Kea6SettingsManager.ensure().
  - >
    Kea v6 does not emit router advertisements — pair it with M(by_systems.opnsense.opnsense_radvd_entry).
options:
  host:
  ...
  interfaces:
    ...
    elements: str
  mac_sources:
    description: MAC address sources (default ipv6-link-local).
    type: list
    elements: str
  state:
...
EXAMPLES = r"""
- name: Enable Kea DHCPv6 on the internal VLANs
  by_systems.opnsense.opnsense_kea6_settings:
    host: "{{ opn_host }}"
    key: "{{ opn_key }}"
    secret: "{{ opn_secret }}"
    enabled: true
...
_LIST_FIELDS = ("interfaces", "mac_sources")
_ENUMS = {}
_API_NAMES = {}
...
def main() -> None:
    ...
    run_module(module, Kea6SettingsManager, params)
