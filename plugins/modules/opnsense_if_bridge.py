...
"""Ansible module for OPNsense bridge interface management."""

from __future__ import annotations

DOCUMENTATION = r"""
---
module: opnsense_if_bridge
short_description: Manage OPNsense bridge interfaces
version_added: "0.2.0"
description:
  - Create, update, or delete bridge interfaces on OPNsense.
  - Thin wrapper around lib-opnsense IfBridgeManager.ensure().
...
options:
  ...
  dedupe:
    ...
    default: false
  descr:
    description: Bridge description (used as unique identifier for matching).
    type: str
    required: true
  state:
    description: Desired state of the bridge.
    type: str
...
- name: Create a bridge interface
  by_systems.opnsense.opnsense_if_bridge:
    host: "{{ opn_host }}"
    key: "{{ opn_key }}"
    secret: "{{ opn_secret }}"
    descr: "Transparent bridge"
    state: present
...
- name: Remove a bridge interface
  by_systems.opnsense.opnsense_if_bridge:
    host: "{{ opn_host }}"
    key: "{{ opn_key }}"
    secret: "{{ opn_secret }}"
...
def main() -> None:
    ...
    run_module(module, IfBridgeManager, params)
