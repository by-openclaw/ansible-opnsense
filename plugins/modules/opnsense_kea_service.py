...
"""Ansible module opnsense_kea_service — thin wrapper on lib-opnsense."""

from __future__ import annotations

DOCUMENTATION = r"""
---
module: opnsense_kea_service
short_description: Control the OPNsense Kea DHCP service
version_added: "0.5.0"
description:
  - Start, stop or reconfigure the Kea DHCP service — lib-opnsense KeaServiceManager.ensure().
  - Reports C(disabled) while both DHCPv4 and DHCPv6 are switched off in their general settings.
...
EXAMPLES = r"""
- name: Kea DHCP — apply configuration
  by_systems.opnsense.opnsense_kea_service:
    host: "{{ opn_host }}"
    key: "{{ opn_key }}"
    secret: "{{ opn_secret }}"
    state: reconfigured
...
def main() -> None:
    ...
    run_module(module, KeaServiceManager, {})
